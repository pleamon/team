...
> **每条 Slack 消息 = 触发对方 Agent 被唤醒 = 一次 LLM 调用 = 花钱。**
> **过度沟通 = 频繁相互触发 = 烧钱 + 低效 + 来回拉扯。**

### 强制执行模式

```
收到任务 → 回复"收到"（1条消息）→ 沉默执行 → 完成后汇报（1条消息）
```

**一个任务只允许 2 条消息：收到 + 完成。** 中间不发消息。
...
- ❌ 遇到小问题就发消息求助（先自己解决）
- ❌ 反复讨论方案不动手（选一个先做）
- ❌ 问"这样做可以吗"（你有权自主决策的事直接做）
- ❌ 转述 A 的话给 B（让他们直接沟通）
- ❌ 回复"好的，收到，我看看"（没有推进的空话）

### 唯一允许中途发消息的情况

- 真正被阻塞（缺依赖、权限不足、任务理解有重大歧义）
- 长任务（>2h）的 Heartbeat 进度更新
